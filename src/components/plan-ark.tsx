...
export function PlanArk() {
  return (
    ...
    <section id="hovedplan" className="plan-ark">
      ...
      >
        <p className="room-number text-[#c62e32]">{hovedplan.rule.stamp}</p>
      ...
      <p className="room-number bg-[#120e0d] px-4 py-4 text-[#c62e32] sm:px-6">{hovedplan.caseAsk}</p>
      <div className="grid lg:grid-cols-3">
        {hovedplan.cases.map((cell, i) => (
          ...
          >
            <p className="room-number text-[#c62e32]">{cell.label}</p>
      ...
      <div className="grid lg:grid-cols-2">
        <PhotoStation
          ...
          cut
        >
          <p className="room-number text-[#c62e32]">Før dere handler</p>
      ...
      <p className="room-number bg-[#120e0d] px-4 py-4 text-[#c62e32] sm:px-6">{hovedplan.solveAsk}</p>
      <div className="grid lg:grid-cols-3">
        {deliveryTracks.map((track, i) => (
          ...
          >
            ...
            <span className="mt-6 inline-flex text-sm font-normal tracking-[0.06em] text-[#c62e32]">
              {track.cta}
            </span>
          </PhotoStation>
        ))}
      ...
      <div className="grid lg:grid-cols-3">
        {hovedplan.solve.map((step, i) => (
          ...
          >
            <p className="room-number text-[#c62e32]">{String(i + 1).padStart(2, "0")}</p>
            <h3 className="mt-3 text-2xl font-medium tracking-tight text-[#fbf8f2]">{step.label}</h3>
            <span className="mt-6 inline-flex text-sm font-normal tracking-[0.06em] text-[#c62e32]">Åpne</span>
          </PhotoStation>
        ))}
      </div>

      <p className="room-number bg-[#120e0d] px-4 py-4 text-[#c62e32] sm:px-6">{hovedplan.whoAsk}</p>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3">
        {commercialCustomers.map((c, i) => (
          <PhotoStation
            ...
            cut={i % 3 !== 2}
          >
            <p className="room-number text-[#c62e32]">{c.room}</p>
      ...
      <div className="grid lg:grid-cols-2">
        <PhotoStation
          ...
          cut
        >
          <p className="room-number text-[#c62e32]">{hovedplan.claimAsk}</p>
          <h2 className="mt-4 max-w-md text-3xl font-medium leading-[1.14] text-[#fbf8f2] sm:text-4xl">
      ...
      >
        <p className="room-number text-[#c62e32]">{hovedplan.goalAsk}</p>
        <ul className="mt-6 grid gap-2 sm:grid-cols-2">
          {hovedplan.goals.map((g) => (
            <li key={g.label}>
              <Link
                href={g.href}
                className="block border-b border-[#fbf8f2]/18 py-2.5 text-[15px] font-normal text-[#fbf8f2] hover:text-[#c62e32]"
              >
                «{g.label}»
              </Link>
            </li>
      ...
      <p className="room-number bg-[#120e0d] px-4 py-4 text-[#c62e32] sm:px-6">Spørsmål vi får</p>
      <div className="grid lg:grid-cols-3">
        {articles.slice(0, 3).map((a, i) => (
          <PhotoStation
            ...
            cut={i < 2}
          >
            <p className="room-number text-[#c62e32]">{a.kicker}</p>
            ...
            <span className="mt-6 inline-flex text-sm font-normal tracking-[0.06em] text-[#c62e32]">Les</span>
          </PhotoStation>
        ))}
      </div>
