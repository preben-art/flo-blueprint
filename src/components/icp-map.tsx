  ...
  return (
    <section id="hvem">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
        <p className="room-number mb-4 text-flo-red">Hva som faktisk gjelder for deg</p>
        <h2 className="max-w-2xl text-2xl font-normal sm:text-4xl">
          Samme bygg. Ulike plikter. Du skal vite din.
        </h2>
        <p className="mt-4 max-w-xl text-[15px] leading-relaxed text-flo-ink">
          ...
          <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {commercialCustomers.map((c) => (
              <Link
                ...
                className="group panel overflow-hidden hover:border-flo-red"
              >
                ...
                <div className="p-4">
                  <p className="room-number text-flo-red">{c.confirmed ? "Navngitt oppdrag" : "Ingen navngitt sak ennå"}</p>
                  <p className="mt-1 text-lg font-normal text-flo-ink">{c.label}</p>
                </div>
              </Link>
            ))}
          </div>
        ...
        <ul className="mt-8 space-y-0 border-t border-flo-ink/18">
          {icpLinks.map((edge) => {
            ...
            return (
              <li
                ...
                className="grid gap-2 border-b border-flo-ink/18 py-5 sm:grid-cols-12 sm:items-baseline"
              >
                <p className="room-number text-flo-red sm:col-span-3">{edge.label}</p>
                <p className="text-sm font-medium sm:col-span-3">
                  {from?.label} og {to?.label}
                </p>
                <p className="text-[15px] leading-relaxed text-flo-ink sm:col-span-6">{edge.meaning}</p>
              </li>
            );
          })}
        </ul>
