        ...
        <div className="plan-read plan-float lg:col-span-5">
          ...
          {showToolLink ? (
            <p className="mt-4 text-sm">
              <Link href="/fag-og-kunnskap/pastand-eller-krav" className="underline hover:text-flo-red">
        ...
        <ol className="ed-rail lg:col-span-7">
          {claimPath.map((step, i) => (
            <RailStep key={step.id} index={i} title={step.label}>
              {!compact ? <p className="mt-1 text-sm leading-relaxed text-flo-muted">{step.meaning}</p> : null}
            </RailStep>
          ))}
        </ol>
      </div>
